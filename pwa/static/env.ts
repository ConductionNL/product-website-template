export const registerEnvironment = () => {
  window.sessionStorage.setItem("GITHUB_REPOSITORY_URL", "https://github.com/CommonGateway/CoreBundle");

  window.sessionStorage.setItem(
    "GITHUB_API_BASE_URL",
    "https://api.github.com/repos/CommonGateway/CoreBundle/contents",
  );

  window.sessionStorage.setItem("READ_THE_DOCS_URL", "https://commongateway.readthedocs.io/en/latest/");

  window.sessionStorage.setItem(
    "FAVICON_URL",
    "data:image/svg+xml;base64,",
  );

  // NavBar
  window.sessionStorage.setItem(
    "SLACK_URL",
    "https://join.slack.com/t/conductionworkspace/shared_invite/zt-1y5kerime-amWEvljvnLvo5S3YfU_jzg",
  );

  window.sessionStorage.setItem(
    "GITHUB_DOCS_DIRECTORY_PATHS",
    '[{"name": "Features", "location": "/docs/features"}, {"name": "CoreBundle Services", "location": "/docs/classes/CommonGateway/CoreBundle/Service"}, {"name": "Docs", "location": "/docs"}, {"name": "Classes", "location": "/docs/classes"}]',
  );

  window.sessionStorage.setItem(
    "NAVBAR_LOGO_URL",
    "https://conduction.nl/wp-content/uploads/2021/07/cropped-conductionlogo-1.png",
  );

  //Jumbotron
  window.sessionStorage.setItem("JUMBOTRON_TITLE", "Common Gateway");

  window.sessionStorage.setItem("JUMBOTRON_SUBTITLE", "Tech to serve people.");

  window.sessionStorage.setItem(
    "JUMBOTRON_DESCRIPTION",
    "Wij zijn wat je zou kunnen noemen Digital Socials, wij ontwikkelen techniek volgens de Common Ground principes, waarbij mens en community centraal staan. Graag dragen we dan ook bij aan het ontwikkelen van digitale oplossingen voor maatschappelijke vraagstukken",
  );

  window.sessionStorage.setItem(
    "JUMBOTRON_SVG",
    "https://www.conduction.nl/wp-content/uploads/2021/07/cropped-Conduction_HOME_0000_afb1-1.png",
  );

  //Footer
  window.sessionStorage.setItem(
    "FOOTER_LOGO_URL",
    "data:image/svg+xml;base64,",
  );
  window.sessionStorage.setItem("FOOTER_LOGO_HREF", "https://conduction.nl");
};
